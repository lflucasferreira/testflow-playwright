...
const projectName = process.env.ALLURE_PROJECT_NAME ?? 'testflow-playwright'
...
function browserFromProjectLabel(result) {
  const parentSuite = result.labels?.find((label) => label.name === 'parentSuite')?.value
  if (!parentSuite) return null

  if (parentSuite.includes('firefox')) return 'firefox'
  if (parentSuite.includes('webkit')) return 'webkit'
  if (parentSuite === 'api' || parentSuite === 'smoke' || !parentSuite.includes('-')) {
    return 'chromium'
  }
...
function ensureBrowserLabel(result, browserHint) {
  ...
  const browser =
    browserHint ?? browserFromProjectLabel(result) ?? normalizeValue(fromParameter) ?? 'unknown'
...
function browserFromResult(result) {
  ...
  const fromProject = browserFromProjectLabel(result)
